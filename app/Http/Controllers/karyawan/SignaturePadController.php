...
class SignaturePadController extends Controller
{
    ...
    {
        $employee = Employee::with(['organization','branch'])->where('employees.id','=',Auth::guard('emp')->user()->id)->first();
        $componentSalary = PayrollEmployeeComponent::where('employee_id', Auth::guard('emp')->user()->id)->pluck('component')->first();
        ...
        if(is_array($arrComponent)){
            ...
            return view('karyawan/signcontract/index',compact('employee','takeHomePay','employeeJoinDate','threeMonthsProbation'));
        }else{
            return back();
        }
    }
    ...
    public function signature(){
        return view('karyawan/signcontract/signature');
    }
