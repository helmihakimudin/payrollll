...
class TimeoffController extends Controller
{
    ...
    public function timeoffbalanceajax(Request $request)
    {
        ...
        $temp = LogTimeOffBalance::join('employees', 'employees.id', '=', 'log_time_off_balances.employee_id')
                ...
                ;
        // $temp = DB::table('status_request_time_off')
        //         ->join('time_off_balances', 'status_request_time_off.time_off_balances_id','=','time_off_balances.id')
        //         ->join('timeoffs', 'timeoffs.id', '=', 'time_off_balances.timeoff_id')
        //         ->join('employees', 'employees.id','=','time_off_balances.employee_id')
        //         ->join('time_off_employees', 'time_off_employees.employee_id','=','employees.id')
        //         ->join('job_position', 'employees.job_position_id','=','job_position.id')
        //         ->join('branches', 'employees.branch_id','=','branches.id')
        //         ->groupBy('timeoffs.id')
        //         ->groupBy('employees.id')
        //         ->orderBy('employees.full_name')
        //         ->select(
        //             'status_request_time_off.id',
        //             'employees.full_name', 
        //             'employees.id as empId', 
        //             'employees.branch_id',
        //             'employees.employee_id', 
        //             'employees.is_active',
        //             'job_position.name as jp_name', 
        //             'branches.name as branch_name', 
        //             'time_off_employees.input_balance', 
        //             'time_off_balances.start_date as top_startdate', 
        //             'time_off_balances.end_date as tob_enddate', 
        //             'time_off_balances.balance_start', 
        //             'time_off_balances.balance_end', 
        //             'time_off_balances.timeoff_id', 
        //             'time_off_balances.created_at',
        //             'time_off_employees.start_date', 
        //             'time_off_employees.end_date',
        //             'timeoffs.name as timeoff_name',
        //             // DB::raw('time_off_balances.balance_end - time_off_balances.balance_start AS cuti')
        //         );
        ...
        if (!empty($boot)) {
            foreach ($boot as  $row) {
                ...
                // dd($carry);
                // get end balance
                // $balance_end = DB::table('log_time_off_balances')->where('employee_id', $row->empId)->where('timeoff_id', $row->timeoff_id)->orderBy('id', 'desc')->first();

                // get timeoff taken
                // $balance_start = DB::table('time_off_balances')->where('employee_id', $row->empId)->where('timeoff_id', $row->timeoff_id)->where('start_date', '>', $beginning_bal->start_date)->where('end_date', '<', $beginning_bal->end_date)->sum('balance_start');
                // dd($balance_start);
                // $balance_ends = DB::table('time_off_balances')->where('employee_id', $row->empId)->where('timeoff_id', $row->timeoff_id)->where('start_date', '>', $beginning_bal->start_date)->where('end_date', '<', $beginning_bal->end_date)->sum('balance_end');
                // dd($balance_end);
                ...
                $obj['adjusment']           = "<div class='text-center text-success font-weight-bold'>". $adjusment ."</div>";
                // $obj['timeoff_taken']       = "<div class='text-danger hover text-center font-weight-bold'>". $timeofftaken ."</div>";
                $obj['carry_forward']       = "<div class='text-center text-success font-weight-bold'>". $carry ."</div>";
                if ($expired != 0) {
                    $obj['expired']         = "<div class='text-danger hover text-center font-weight-bold'>".$expired."</a>";
                } else {
                    $obj['expired']         = "<div class='text-danger hover text-center font-weight-bold'>".$expired."</a>";
                }  
                $obj['ending_balance']      = "<div class='text-center text-success font-weight-bold'>".$ending_bal->ending_balance."</div>";
                // ".isset($beginning_bal->input_balance) ? $beginning_bal->input_balance: '-'."
                // '<a data-toggle="popover" title="Popover title" data-content="Right?">'.($balance_ends-$balance_start).'</a>'
                $obj['timeoff_taken']       = 
                '<div data-html="true" data-toggle="popover" class="text-danger hover text-center font-weight-bold" title="Transaction of <strong>'.$row->timeoff_name.'</strong>" data-content="<strong>'.$row->full_name.'</strong> takes <strong>'. $timeofftaken.' days </strong> of <strong>'.$row->timeoff_name.' </strong> for <strong>'.date("d M Y",strtotime($row->start_date)).'</strong> - <strong>'.date("d M Y",strtotime($row->end_date)).'</strong><br><hr><small>Created at : '.date("d M Y",strtotime($row->created_at)).'</small>">'.$timeofftaken.'</div>';

                // $obj['ending_balance']      = "<div class='text-center text-success font-weight-bold'>$balance_end->balance_end</a>";
    ...
    public function logbalanceajax(Request $request)
    {
        ...
        // $tempA = TimeOffEmployee::join('timeoffs', 'timeoffs.id', '=', 'time_off_employees.timeoff_id')
        //         ->join('employees', 'time_off_employees.employee_id', '=', 'employees.id')
        //         ->join('job_position', 'job_position.id', '=', 'employees.job_position_id')
        //         ->join('branches', 'branches.id', '=', 'employees.branch_id')
        //         ->select('employees.id', 'time_off_employees.created_at', 'employees.employee_id', 'employees.full_name', 'job_position.name as jp_name', 'branches.name as branch_name', 'timeoffs.name as timeoff_name', 'time_off_employees.type as transactions', 'time_off_employees.input_balance as balance');
        //         if ($request->toDate2 && $request->fromDate2 && $request->selectTimeoff2) {
        //             $tempA = $tempA->whereDate('time_off_employees.created_at', '>=', $request->fromDate2)
        //             ->whereDate('time_off_employees.created_at', '<=', $request->toDate2)
        //             ->where('time_off_employees.timeoff_id', $request->selectTimeoff2);
        //         }
        //         if ($request->input('select_employee')) {
        //             $tempA->whereIn('employees.id', $request->input('select_employee'));
        //         }
        // $temp = DB::table('time_off_balances')
        //         ->join('timeoffs', 'timeoffs.id', '=', 'time_off_balances.timeoff_id')
        //         ->join('employees', 'time_off_balances.employee_id', '=', 'employees.id')
        //         ->join('job_position', 'job_position.id', '=', 'employees.job_position_id')
        //         ->join('branches', 'branches.id', '=', 'employees.branch_id')
        //         ->join('status_request_time_off', 'time_off_balances.id', '=', 'status_request_time_off.time_off_balances_id')
        //         ->select('employees.id', 'time_off_balances.created_at', 'employees.employee_id', 'employees.full_name', 'job_position.name as jp_name', 'branches.name as branch_name', 'timeoffs.name as timeoff_name', 'status_request_time_off.status as transactions', DB::raw('DATEDIFF(time_off_balances.end_date,time_off_balances.start_date)+1 AS balance'))
        //         ->unionAll($tempA);
        ...
        if (!empty($boot)) {
            foreach ($boot as  $row) {
                ...
                if ($row->type == 'time_off_taken' || $row->type == 'expired') {
                    $obj['balance']         = "<div class='text-center text-danger'>$row->value</div>";
                } else {
                    $obj['balance']         = "<div class='text-center text-success'>$row->value</div>";
                }
                
                if ($row->action == 'Expired') {
                    $obj['detail']          = '<div data-html="true" data-toggle="popover" class="text-danger hover text-center font-weight-bold" title="<strong>'.$row->action.'</strong>" data-content="'.$row->value.' days '.$row->timeoff_name.' of '.$row->full_name.' is expired by sistem">View Detail</div>';
                } else {
                    $obj['detail']          = '<div data-html="true" data-toggle="popover" class="text-danger hover text-center font-weight-bold" title="<strong>'.$row->action.'</strong>" data-content="'.$row->value.' days '.$row->timeoff_name.' '.$row->full_name.' is '.$row->action.' by sistem <br><hr>expired date : '.$row->end_date.'">View Detail</div>';
                }
